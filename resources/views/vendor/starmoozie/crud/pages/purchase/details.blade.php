<thead>
    <tr>
        ...
        <th>{{ __('starmoozie::title.amount_profit') }}</th>
        <th>{{ __('starmoozie::title.calculated_profit') }}</th>
        <th>{{ __('starmoozie::title.total_profit') }}</th>
    </tr>
</thead>
<tbody>
    @foreach($column['value'] as $key => $value)
        ...
        <tr>
            <td>{{ $value->name }}</td>
            <td>{{ rupiah($pivot->qty) }}</td>
            <td>{{ rupiah($pivot->buy_price) }}</td>
            <td>{{ rupiah($pivot->sell_price) }}</td>
            <td>{{ rupiah($pivot->total_price) }}</td>
            <td>{{ __("starmoozie::title.{$type_profit}") }}</td>
            <td>{{ rupiah($pivot->amount_profit) }}</td>
            <td>{{ rupiah($pivot->calculated_profit) }}</td>
            <td>{{ rupiah($total_profit) }}</td>
        </tr>
    @endforeach
</tbody>
